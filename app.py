...
#   X API      -> external_apis.x_api      -> ai_pipeline
#   Reddit API -> external_apis.reddit_api -> ai_pipeline
#   Manual URL -> manual_sources.manual_external -> ai_pipeline
import external_apis.x_api as x_api
import external_apis.reddit_api as reddit_api
import manual_sources.manual_external as manual_external
...
def x_analyze():
    ...
    try:
        ...
        max_results = 50
    try:
        # v15.9: via external_apis.x_api so stored rows carry source_type="X_API".
        result = x_api.fetch(max_results=max_results)
    except x_api.XFetchError as e:
        return api_error(e.message_en, e.message_ar, e.status)
...
def reddit_analyze():
    ...
    try:
        ...
        max_comments = 8
    try:
        # v15.9: via external_apis.reddit_api so rows carry source_type="Reddit_API".
        result = reddit_api.fetch(
            max_posts=max_posts, max_comments_per_post=max_comments)
    except reddit_api.RedditFetchError as e:
        return api_error(e.message_en, e.message_ar, e.status)
    except Exception as e:
...
#   Manual link -> manual_sources.manual_external -> ai_pipeline
# ---------------------------------------------------------------- #
